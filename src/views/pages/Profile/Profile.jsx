...
const ProfilePage = () => {
  ...
  const getProfileDetails = async () => {
    ...
    try {
      const res = await axios.post(`${URL}profile`, userData);
  ...
  const updateProfileDetails = async (e) => {
    ...
    try {
      const res = await axios.put(`${URL}update-profile`, userData);
      setisLoading(false);
      if (res.data.status === "success") {
  ...
  const updatePassword = async (e) => {
    ...
    if (new_password === confirmNewPassword) {
      try {
        const res = await axios.patch(`${URL}update-password`, userData);
        setisLoading(false);
        if (res.data.status === "success") {
          toast.success(res.data.message, {
            position: "bottom-left",
      ...
      <div className="profile">
        <div className="sidebar">
          <ul className="">
            ...
            <li className="" onClick={() => setdisplayScreen("overview")}>
              {" "}
              <FaUserAlt /> <p>My Account</p>
            </li>
            <li className="" onClick={() => setdisplayScreen("orders")}>
              <FaBox /> <p>Orders</p>
            </li>
            <li className="" onClick={() => setdisplayScreen("edit-profile")}>
              <FaUserEdit /> <p>Edit Profile</p>
            </li>
            <li className="" onClick={() => setdisplayScreen("address")}>
              <FaAddressBook /> <p>Address Book</p>
            </li>
            <li
              className=""
              onClick={() => setdisplayScreen("change-password")}
            >
              <FaUnlockAlt /> <p>Change Password</p>
            </li>
          </ul>
        </div>
                        ...
                        <div className="">
                          <div className="order-detail">
                            ...
                            <div className="order-info">
                              ...
                              <h6 className="">Date: 22-12-2022</h6>
                            </div>
                          </div>
                        </div>
                        <span
